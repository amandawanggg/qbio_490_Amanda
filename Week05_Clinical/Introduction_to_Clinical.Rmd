...
colnames(mtcars)
rownames(mtcars)# fill in name of carburetor column here
```
...
write.csv(clinic, "/home1/wangaman/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/wangaman/490_cluster/analysis_data/brca_clinical_data.csv")
```

```{r}
clinic_full <- clinical.BCRtab.all$clinical_patient_brca
colnames(clinic_full)
```
...
There are 112 columns, 1099 rows
...
18371 + 158= 18529
18529 /365 = 50.76 years old

```{r Exercise 3.1}
class(clinic_full$gender)
```

View the column and row names. What two commands should you use?

```{r Exercise 3.2}
clinic_full[clinic_full$bcr_patient_barcode == "TCGA-4H-AAAK", "age_at_diagnosis"]
```

View the vital status column. What two notations can be used to view columns? 

```{r Exercise 3.3}
patient1 <- clinic_full[clinic_full$bcr_patient_barcode == "TCGA-4H-AAAK", c("birth_days_to", "last_contact_days_to")]
head (patient1)
```
...
```{r Exercise 4.1}
# create plot here
plot(clinic_full$birth_days_to, clinic_full$last_contact_days_to,
     xlab = "birth_days_to",
     ylab = "last_contact_day_to")
...
```{r Exercise 4.2}
# create plot here
boxplot(as.numeric(clinic_full$last_contact_days_to) ~ clinic_full$race,
     ylab = "last_contact_day_to")
```
...
vital_status_na_mask <- !is.na(clinic_full$vital_status)
clinic_full_clean <- clinic_full[vital_status_na_mask,]
```

2) Create a mask to select for male patients
```{r}
male_mask <- clinic_full_clean$gender == "MALE"
```

3) Create a mask to select for patients under the age of 35 OR over 60 at diagnosis. 
```{r}
under30_over60_mask <- as.numeric(clinic_full_clean$age_at_diagnosis) < 35 | as.numeric(clinic_full_clean$age_at_diagnosis) > 60
```
...
```{r}
no_middle_clinic <- clinic_full_clean[under30_over60_mask,]
```
...
```{r}
age_NA_mask <- is.na(clinic_full$age_at_diagnosis)
cleaned_clinic <- !is.na(clinic_full$age_at_diagnosis)
```
...
```{r}
cleaned_clinic$age_category <- ifelse((clinic_full$age_at_diagnosis) <=35, "Young",                                 ifelse((clinic_full$age_at_diagnosis) >35 & (clinic_full$age_at_diagnosis) <50 , "Middle", "Old"))
```
...
```{r}
cleaned_clinic$survival_time <- ifelse(!is.na((clinic_full$last_contact_days_to)), (clinic_full$last_contact_days_to), (clinic_full$death_days_to)) 
```
...
```{r}
cleaned_clinic$death_event <- ifelse(clinic_full$vital_status== "Alive", T, F)
```
...
y axis represent the survival probability
# 2. What does the x axis represent?
x axis represent the time patient alive
# 3. What age group appears to have the greatest rate of survival?
The blue line has slightly higher than the other 2 color, so the young group. But towards the end, as time increases, middle age survival rate is still greater than the old group
# 4. What age group appears to have the worst rate of survival?
old group has the worst survival rate as it continues to be the lowest one among other groups.
...
The p value is 0.0068 which is less than 0.05, so that it rejects the null hypothesis. There is a significant difference in the survival rate between the age groups. 
